...
public class IOLib extends Version {
   ...
   static public String getStringFromInput(InputStream in) {
       StringBuilder sw = new StringBuilder();
       try {
   ...
   static public String getStringFromHTTP(String key, String payload) {
       StringBuilder sw = new StringBuilder();
       String uKey=key.toLowerCase()+"=";
       for ( String s : payload.split("&") ) {
           if ( s.toLowerCase().startsWith(uKey)) {
               String val = s.substring(uKey.length());
